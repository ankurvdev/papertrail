...
class Resource
{
    ...
    const gsl::span<const std::byte>& data() const { return _data; }
    template <typename T> auto        data() const
    {
        ...
        return gsl::span<const T>{ptr, ptr + size};
    }
    ...
    Resource(const std::wstring_view name, const gsl::span<const std::byte> data) : _name(name), _data(data) {}

    private:
    std::wstring_view          _name;
    gsl::span<const std::byte> _data;
};
...
class Collection
{
    ...
    Collection(gsl::span<Resource::GetFunc* const> fnTable) : _fnTable(fnTable) {}

    size_t   Size() const { return _fnTable.size(); }
    Resource GetAt(const size_t index) const { return (*_fnTable.at(index))(); }

    Iterator begin() const { return Iterator(*this); }
    Iterator end() const { return Iterator(); }

    private:
    gsl::span<Resource::GetFunc* const> _fnTable;
};
}    // namespace EmbeddedResource
